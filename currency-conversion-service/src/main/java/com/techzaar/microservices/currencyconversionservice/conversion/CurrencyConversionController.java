...
public class CurrencyConversionController {
  ...
  public CurrencyValueBean getCurrencyConvertedValue(@PathVariable String from,
    ...
    ResponseEntity<CurrencyValueBean> currencyValueResponse = new RestTemplate().getForEntity(
        "http://localhost:8000/currency-exchange/from/{from}/to/{to}", CurrencyValueBean.class,
        parameterMap);
  ...
  public CurrencyValueBean getCurrencyConvertedValueFeign(@PathVariable String from,
      @PathVariable String to, @PathVariable BigDecimal quantity) {
    ...
    CurrencyValueBean returnval =
        new CurrencyValueBean(value.getId(), from, to, value.getConversionMultiple(), quantity,
            quantity.multiply(value.getConversionMultiple()), value.getPort());
